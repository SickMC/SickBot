package me.anton.sickbot.managing
...
class Rules {
    ...
    private fun sendRuleMessage(channel: MessageChannel) {
        scope.launch {
            channel.createEmbed {
    ...
    private fun handleInteraction(){
        SickBot.instance.kord.on<ButtonInteractionCreateEvent> {
            ...
            if (!SickBot.instance.mainGuild.getMember(interaction.user.id).roleIds.contains(SickBot.instance.getIdOfRankGroup("Player")))return@on
            SickBot.instance.mainGuild.getMember(interaction.user.id).addRole(SickBot.instance.getIdOfRankGroup("Player"), "Accepted the rules!")
        }
    }

    private fun handleRuleCreate(){
        SickBot.instance.kord.events
            .filterIsInstance<MessageCreateEvent>()
            .map { it.message }
            .filter { it.author?.isBot == false }
            .filter { it.content == "!rules" }
            .filter { it.author!!.id == SickBot.instance.getIdOfRankGroup("Administration") }
            .onEach {
                sendRuleMessage(it.channel.fetchChannel())
                it.delete("Rule message was send")
            }
            .launchIn(SickBot.instance.kord)
    }
